...
/**
 * Classe que representa una expressió booleana mínima, on l'operador relacional és el major igual
 * És un subtipus de EBRelacional
 *@author Laia Riera Guerra
 * @version v.4 05/05/2007
 */
public class EsMajorIgual extends EBRelacional{
	/**
	 * Creadora buida d'EsMajorIgual
	 *
	 */
	public EsMajorIgual(){
		super();
	...
	/**
	 * Creadora d'EsMajorIgual, on posa com a variable el paràmetre d'entrada <code>vari</code>,
	...
	public EsMajorIgual(Propietat vari, Object oValor,boolean esPropietat){
		super(vari, oValor,esPropietat);
	}//endofmethod
	
	/**
	 * Avalua una expressió booleana, el nucli de la qual és l'operador major igual
	 * <p>
			...
			for(int i=0;i<d.length;i++){
				if(ovar instanceof PropCategorica){
					...
					if(sDada.compareTo("?")==0)resultat.add(i,"?");
						...
						Float fdada=new Float((String)d[i].obtenirValor().toString());
						//Float fdada=(Float)d[i].obtenirValor();
	 ...
	 */
	 public String escriureNormal(){
		 System.out.println("Esta boig a anat a escriureNormal");
		  Propietat p=this.getVar();
		  String s;
		  if(this.isValorEsPropietat()){
			  s="("+p.obtenirId()+") "+this.getMAJORIGUAL()+" ("+this.getValor()+")";
		  }else s="("+p.obtenirId()+") "+this.getMAJORIGUAL()+" "+this.getValor();	
		  return s;
	  }
	 /**
	 ...
	 public String escriureNormalLatex(){
		  ...
		  String s;
		  if(this.isValorEsPropietat()){
			  s=fTex.adaptarATex(p.obtenirId())+" \\geq "+fTex.adaptarATex((String)this.getValor());			  
		  }else{
			  Float f=(Float)this.getValor();
			  s=fTex.adaptarATex(p.obtenirId())+ " \\geq "+fTex.formatejarReal(f.floatValue());		  
		  }
		  return s;
	  }
	 /***
	 ...
	 public ExpressioBooleana llegirBCNormalRelacional(String sOperand,String sProp,String sValor, LlistaPropietats llistaProp,boolean esPropietat)throws Exception{
		 ...
		 if(sOperand.compareTo(ExpressioBooleana.MAJORIGUAL) == 0){
			 if(esPropietat==true){
				  ...
				  else return new EsMajorIgual(p,oValor,esPropietat);			  
			  }else{
