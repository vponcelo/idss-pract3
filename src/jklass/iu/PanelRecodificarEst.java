...
*/
public class PanelRecodificarEst extends javax.swing.JPanel {
  ...
  // JList jListVarsN = new JList(listModelVN);//ale
	...
	MouseListener mouseRegla =new MouseAdapter(){
	  ...
	  */
	public PanelRecodificarEst(FrPrincipal fr,GestorKlass gk) {
		super();
		frPare = fr;
		gestor = gk;	
		IS_OK=true;
	...
	private void initGUI() {
		try {
			frPare.setSize(670, 510);
			frPare.centrar();
			this.setPreferredSize(new java.awt.Dimension(675, 430));
			...
			this.setBorder(BorderFactory.createTitledBorder(null, "Definir Estados ale", TitledBorder.LEADING, TitledBorder.TOP, new java.awt.Font("Dialog",0,10)));
			{
				...
				}
				{
					...
					jPanelButtons.setLayout(null);
					{
						...
						jPanelButtons.add(jButtonAfegirMod);//Primer mano pone
				...
				}
				{
					...
					jPanel3.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(BevelBorder.LOWERED), "Variable resultat", TitledBorder.LEADING, TitledBorder.TOP, new java.awt.Font("Dialog",0,10)));
					{
						...
						}
						{
							...
							jTextFieldNomVarNova.setBounds(50, 7, 80, 21); //Estat
	...
	System.out.println( "este las lleva a la izquierda eh? sabado");
		try{
			Object ob=jListModsNoves.getSelectedValue();
	    	jTextFieldNomModalitatNova.setText((String)ob);	     		
	 		int imod=0;
	 		boolean b=false;
			System.out.println( "alnomModalitats está vacio sabado= "+alnomModalitats.size());
	 		for(int i=0;i<alnomModalitats.size() && !b;i++){
	 			String saux=(String)alnomModalitats.get(i);
					System.out.println( "que hay en alnomModalitatsmio sabado "+saux);
	 			if(saux.compareTo((String)ob)==0){
	 				imod=i;
	 				b=true;
	 			}
	...
	protected void jButtonTreureMod_actionPerformed(ActionEvent e) {
		IS_OK=false;
		 ...
		 Object[] vars = jListModsSelecc.getSelectedValues();
		    ...
		    int n = 0;
			  System.out.println( "este es el sacar bolasssss?"); //este NO las saca
		    do {
		      try {
