...
/**
 * Classe que representa una expressió booleana mínim, on l'operador relacional és el major
 * És un subtipus de EBRelacional
 *@author Laia Riera Guerra
 * @version v.4 05/05/2007
 */

public class EsMajor extends EBRelacional{
	/**
	 * Creadora buida d'EsMajor
	 *
	 */
	public EsMajor(){
		super();
	...
	/**
	 * Creadora d'EsMajor, on posa com a variable el paràmetre d'entrada <code>vari</code>,
	 ...
	 */
	public EsMajor(Propietat vari, Object oValor,boolean esPropietat){
		super(vari, oValor,esPropietat);
	}//endofmethod
	
	/**
	 * Avalua una expressió booleana, el nucli de la qual és l'operador major
	 * <p>
	 ...
	 */
	 public String escriureNormal(){
		  ...
		  if(this.isValorEsPropietat()){
			  s="("+p.obtenirId()+") "+this.getMAJOR()+" ("+this.getValor()+")";
		  }else s="("+p.obtenirId()+") "+this.getMAJOR()+" "+this.getValor();		  
		  return s;
	  }
	 /**
	 ...
	 public String escriureNormalLatex(){
		  ...
		  String s;
		  if(this.isValorEsPropietat()){
			  s=fTex.adaptarATex(p.obtenirId())+">"+fTex.adaptarATex((String)this.getValor());			  
		  }else{
			  Float f=(Float)this.getValor();
			  s=fTex.adaptarATex(p.obtenirId())+">"+fTex.formatejarReal(f.floatValue());		  
		  }
		  return s;
	  }
	 /***
	 ...
	 public ExpressioBooleana llegirBCNormalRelacional(String sOperand,String sProp,String sValor, LlistaPropietats llistaProp,boolean esPropietat)throws Exception{
		 ...
		 if(sOperand.compareTo(ExpressioBooleana.MAJOR) == 0){
			 if(esPropietat==true){
				  ...
				  else return new EsMajor(p,oValor,esPropietat);			  
			  }else{
