...
/**
 * Classe que representa una expressió booleana mínima, on l'operador relacional és el menor
 * És un subtipus de EBRelacional
 *@author Laia Riera Guerra
 * @version v.4 05/05/2007
 */
public class EsMenor extends EBRelacional{
	/**
	 * Creadora buida d'EsMenor
	 *
	 */
	public EsMenor(){
		super();
	...
	/**
	 * Creadora d'EsMenor, on posa com a variable el paràmetre d'entrada <code>vari</code>,
	 ...
	 */
	public EsMenor(Propietat vari, Object oValor,boolean esPropietat){
		super(vari, oValor,esPropietat);
	}//endofmethod
	
	/**
	 * Avalua una expressió booleana, el nucli de la qual és l'operador menor
	 * <p>
	 ...
	 */
	 public String escriureNormal(){
		  ...
		  if(this.isValorEsPropietat()){
			  s="("+p.obtenirId()+") "+this.getMENOR()+" ("+this.getValor()+")";
		  }else s="("+p.obtenirId()+") "+this.getMENOR()+" "+this.getValor();		
		  return s;
	  }
	 /**
	 ...
	 public String escriureNormalLatex(){
		  ...
		  String s;
		  if(this.isValorEsPropietat()){
			  s=fTex.adaptarATex(p.obtenirId())+" "+this.getMENOR()+" "+fTex.adaptarATex((String)this.getValor());			  
		  }else{
			  Float f=(Float)this.getValor();
			  s=fTex.adaptarATex(p.obtenirId())+" "+this.getMENOR()+" "+fTex.formatejarReal(f.floatValue());		  
		  }
		  return s;
	  }
	 /***
	 ...
	 public ExpressioBooleana llegirBCNormalRelacional(String sOperand,String sProp,String sValor, LlistaPropietats llistaProp,boolean esPropietat)throws Exception{
		 ...
		 if(sOperand.compareTo(ExpressioBooleana.MENOR) == 0){
			 if(esPropietat==true){
				 System.out.println("Propietat valor = "+sValor);
				 try{
					 Propietat pValor=llistaProp.obtenirPropietat(sValor);
					 if(llistaProp.esPropietatNumerica(sProp)&& !llistaProp.esPropietatNumerica(sValor))throw new Exception("No es pot comparar una variable numèrica amb una que no ho és");
					 ...
					 else return new EsMenor(p,oValor,esPropietat);		
				 }catch(Exception ex){
					 throw new Exception("No existeix la propietat de nom " + sValor);
				 }				  	  
			  }else{
